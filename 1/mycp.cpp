...
void executeCommand(const string& program, const string& strategy, int input_fd, int output_fd) {
    int pipeIn[2];
    int pipeOut[2];
    pid_t pid;
    ...
    if (pid == 0) { // Child process
        ...
        cout << "Executing program: " << program << " with strategy: " << strategy << endl;
        char *args[] = {strdup(program.c_str()), strdup(strategy.c_str()), NULL};
        execv(args[0], args);
        perror("execv");
        exit(EXIT_FAILURE);
    } else { // Parent process
...
void handleDirectIO(int input_fd, int output_fd) {
    char buffer[1024];
    while (true) {
        int valread = read(input_fd, buffer, sizeof(buffer));
        if (valread == 0) {
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 3; i < argc; ++i) {
        ...
        if (arg == "-i" && i + 1 < argc) {
            ...
            inputMethod = outputMethod = argv[++i];
            cout << "Both input and output method: " << inputMethod << endl;
        }
    }
    ...
    executeCommand(programName, strategy, input_fd, output_fd);
